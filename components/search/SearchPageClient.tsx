        ...
        <div className="mt-8 grid gap-8">
          ...
          <section className="space-y-3">
            <div className="flex items-end justify-between gap-3">
              ...
              {(summaryLoading || newsLoading) && (
                <span className="text-xs text-(--text-muted)">생성 중…</span>
              )}
            </div>
            ...
            {!summaryLoading &&
              ...
              (!summaryData.insights || summaryData.insights.length === 0) && (
                <div className="p-5 rounded-xl border border-(--border) bg-(--card-bg)">
                  <p className="text-(--text-body)">아직 인사이트가 없습니다.</p>
                </div>
              )}
          </section>
          ...
          <section className="space-y-3">
            ...
            {showKoreanNews && translateError && (
              <div className="p-5 rounded-xl border border-(--border) bg-(--card-bg)">
                <p className="text-(--text-body)">기사 번역에 실패했습니다.</p>
                <p className="text-xs text-(--text-muted) mt-2">
                  {translateError}
                </p>
              </div>
            ...
            <div className="grid gap-3">
              {newsData?.articles?.map((a) => (
                ...
                >
                  <p className="text-sm text-(--text-muted)">{a.source?.name}</p>
