  ...
  return (
    <section className="kakao-card overflow-hidden p-0">
      <div className="grid md:grid-cols-[1fr_auto_1.2fr]">
        <div className="p-8">
          <h3 className="text-lg font-bold text-(--text-title)">전략 포트폴리오</h3>
          <p className="mt-1 text-sm font-medium text-slate-500">AI 모델 기반 최적화 비중</p>
          
          <div className="mt-8 space-y-6">
            {performance.map((item) => (
              <div key={item.label} className="flex items-end justify-between border-b border-slate-50 pb-4 dark:border-slate-800">
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">{item.label}</p>
                  <p className="mt-1 text-2xl font-black text-(--text-title)">{item.value}</p>
                </div>
        ...
        <div className="bg-(--light-bg) p-8 dark:bg-slate-900/30">
          <h4 className="text-sm font-black uppercase tracking-widest text-slate-400">Major Holdings</h4>
          
          <div className="mt-6 space-y-5">
            {holdings.map((holding) => (
              <div key={holding.name} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-(--text-title)">{holding.name}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black text-(--text-body)">{holding.ratio}</span>
                    <span className="text-[10px] font-bold text-emerald-500">{holding.change}</span>
                  </div>
                </div>
